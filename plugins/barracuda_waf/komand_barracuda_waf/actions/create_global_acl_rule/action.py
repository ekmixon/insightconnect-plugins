...
class CreateGlobalAclRule(komand.Action):
    ...
    def run(self, params={}):
        ...
        action = f"{action}/" + params.get("policy_id") + "/global_acls"

        r = self.connection.connector.post(
            action,
            self.connection.connector.get_dict_from_params(
                params.get("global_acl"), ["name", "extended_match", "extended_match_sequence"]
            ),
        )
