...
import json
import requests
from .schema import RebuildDropletInput, RebuildDropletOutput


class RebuildDroplet(komand.Action):
    def __init__(self):
        super(self.__class__, self).__init__(
            name="rebuild_droplet",
            description="Rebuilds the droplet from a specified image",
            input=RebuildDropletInput(),
            output=RebuildDropletOutput(),
        )
    ...
    def run(self, params={}):
        ...
        payload = {"type": "rebuild", "image": image_id}
