...
class TestAdmin(unittest.TestCase):
    def setUp(self):
        self.client = duo_client.admin.Admin("test_ikey", "test_akey", "example.com")
        ...
        self.client_list = duo_client.admin.Admin("test_ikey", "test_akey", "example.com")
        self.client_list.account_id = "DA012345678901234567"
        self.client_list._connect = lambda: util.MockHTTPConnection(data_response_should_be_list=True)

        # if you are wanting to get a response from a call to get
        # authentication logs
        self.client_authlog = duo_client.admin.Admin("test_ikey", "test_akey", "example.com")
        self.client_authlog.account_id = "DA012345678901234567"
        self.client_authlog._connect = lambda: util.MockHTTPConnection(data_response_from_get_authlog=True)
    ...
    def test_get_users_by_name(self):
        ...
        self.assertEqual(util.params_to_dict(args), {"username": ["foo"], "account_id": [self.client.account_id]})
        self.assertEqual(response["body"], None)
        response = self.client.get_users_by_name("foo")
        (uri, args) = response["uri"].split("?")
        self.assertEqual(response["method"], "GET")
    ...
    def test_update_user(self):
        ...
        self.assertEqual(response["method"], "POST")
        self.assertEqual(response["uri"], "/admin/v1/users/DU012345678901234567")
        self.assertEqual(
            util.params_to_dict(response["body"]),
            {
                "account_id": [self.client.account_id],
