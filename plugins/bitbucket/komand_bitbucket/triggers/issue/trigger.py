...
class Issue(komand.Trigger):
    ...
    def run(self, params={}):
        ...
        issue_params = [
            ...
            (
                'and kind = "%s"',
                ("bug" if params.get(Input.KIND) == "None" else params.get(Input.KIND)).lower(),
            ),
            ...
            (
                'and state = "%s"',
                ("new" if params.get(Input.STATE) == "None" else params.get(Input.STATE)).lower(),
            )
        ...
        while True:
            ...
            try:
                ...
                api_call = (
                    ...
                    + f"/repositories/{self.connection.username}/{params.get(Input.REPOSITORY).lower()}/issues?{query} "
                )
