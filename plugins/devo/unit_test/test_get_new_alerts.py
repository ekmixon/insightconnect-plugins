...
from icon_devo.triggers.get_new_alerts import GetNewAlerts
import logging
import json
...
class TestGetNewAlerts(TestCase):
    ...
    def test_integration_get_new_alerts(self, mockSend):
        ...
        test_alert_received = GetNewAlerts()
        test_alert_received.connection = test_connection
        test_alert_received.logger = log

        test_alert_received.run(trigger_params)
