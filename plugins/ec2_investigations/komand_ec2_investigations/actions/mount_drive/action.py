...
class MountDrive(komand.Action):
    ...
    def run(self, params={}):
        ...
        try:
            ...
            reservations = self.connection.aws.get_all_instances(
            ...
            if stdout.decode("utf-8").rstrip() == "0":
                ...
                result = json.loads(
                    '{"directory": "%s", "status": "Directory already mounted"}' % directory
                )
                self.logger.info(
                    "Unable to mount directory: %s. Directory already mounted", directory
                )
            ...
            self.logger.error(
                "No address associated with hostname %s. Verify instance is running and credentials are valid",
                instance_id,
            )
            raise
