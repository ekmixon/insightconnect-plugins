...
class HashLookup(komand.Action):
    ...
    def run(self, params={}):
        ...
        if response.status_code == 403:
            ...
            raise Exception(
                "The account configured in your plugin connection is currently rate-limited. Adjust the time between requests in the plugin action configuration if possible or consider adding a Sleep plugin step between attempts."
            )
        elif response.status_code == 503:
            raise Exception(
                "Server error occurred. Verify your plugin connection inputs are correct and not malformed and try again. If the issue persists, please contact support."
            )
